...
- **`apps/dashboard` uses the publishable key.** It reads `VITE_SUPABASE_PUBLISHABLE_KEY` only - it
  deploys from this workspace, where the environment is known. An app scaffolded outside this repo
  should keep a `VITE_SUPABASE_ANON_KEY` fallback, because whoever ships it may not have migrated
  yet. Never put a secret key in a `VITE_`-prefixed variable: Vite inlines those into the bundle.
