...
- The application follows a modular architecture with feature modules in
  `src/modules/{module-name}/`
- Each module must contain components, composables, router, services, stores, and views directories
- Module routes are aggregated in `src/router/index.ts` and must follow the `{module}Routes` naming
  convention
...
- Components consuming theme context must have a parent with the provider (enforced in
  `useAppTheme`)
...
- Native features like back button, keyboard, and splash screen are managed through dedicated
  handlers
