...
- The back button logic prioritizes closing modals (actions, dialogs, sheets, popups, login screens)
  before navigating back
- If no modals are open, it checks for searchbar, expandable cards, or view history before
  potentially exiting the app
...
- The `assetsInlineLimit: 0` setting in vite.config.ts means no assets are inlined, which can affect
  loading performance
