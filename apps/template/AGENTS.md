...
- Use `@modules/*` alias for paths relative to `src/modules/*` directory (configured in
  vite.config.ts)
- Import types with `import type` syntax for TypeScript type imports
- Auto-imported functions are configured via `unplugin-auto-import` and listed in
...
- Framework7 components can be used in both PascalCase (F7Page, F7Navbar) and kebab-case (f7-page,
  f7-navbar)
- The `Framework7VueResolver()` in `src/shared/utils/resolvers/resolvers.ts` handles the component
  resolution
...
- The `useAppTheme.ts` file is missing imports for `computed`, `provide`, `inject`, `onMounted`, and
  `useLocalStorage`
